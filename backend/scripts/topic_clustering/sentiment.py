...
from topic_clustering.nlp import nlp
...
def process_tweet(data):
    ...
    with open(os.path.join('topic_clustering/data', 'tokens.pkl'), 'wb') as f:
        pickle.dump(X, f)
    with open(os.path.join('topic_clustering/data', 'Y.pkl'), 'wb') as f:
        pickle.dump(Y, f)
    return (X, Y)
...
def get_word_features(wordlist, vocab_size=3000):
    ...
    with open(os.path.join('topic_clustering/data', 'words.txt'), 'w') as f:
...
def train_model(X, Y, model, load_word_features):
    ...
    filename = os.path.join('topic_clustering/data', 'word_features.pkl')
    word_features = None
    ...
    with open(os.path.join('topic_clustering/data', '{}.pkl'.format(model)), 'wb') as f:
...
def predict(tweets, model='SVC', debug=False):
    ...
    word_features_file = open(os.path.join('topic_clustering/data', 'word_features.pkl'), 'rb')
    ...
    # Get model
    model_file = open(os.path.join('topic_clustering/data', '{}.pkl'.format(model)), 'rb')
...
def main(
    ...
    if load_training_data:
        ...
        with open(os.path.join('topic_clustering/data', 'tokens.pkl'), 'rb') as f:
            X = pickle.load(f)
        with open(os.path.join('topic_clustering/data', 'Y.pkl'), 'rb') as f:
            Y = pickle.load(f)
    # start to train
